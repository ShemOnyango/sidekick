...
const TripSummaryScreen = ({ navigation, route }) => {
  ...
  const loadTripData = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/authorities/${authorityId}`
      );
      const authority = await response.json();

      // Fetch pins for this trip
      const pinsResponse = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/pins/authority/${authorityId}`
      );
  ...
  const exportToEmail = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/email/send-trip-summary`,
        {
          method: 'POST',
  ...
  const exportToPDF = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/trip/generate/${authorityId}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const generateEmailHTML = () => {
    return `
      ...
      <body>
        ...
        <div class="footer">
          <p>Generated by Herzog Rail Authority System</p>
